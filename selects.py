import sqlite3 #import SQL stuff
connection = sqlite3.connect('dogs.db') #connect the SQL library to the database dog
cursor = connection.cursor() #Cursor is a database object to retrieve data from
                             # a result set one row at a time, instead of the T-SQL
                             #commands that operate on all the rows in the result
                             #set at one time. We use cursor when we need to update
                             #records in a database table in singleton fashion
                             #means row by row.
# creation = open('create.sql', 'r').read()
